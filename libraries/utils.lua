...
functions.GetAllTargets = function(distance: number?, mobs: boolean?, sort: (table, table) -> (boolean?))
	...
	for i,v in players:GetPlayers() do 
		if v ~= lplr and functions.isAlive(v, game.PlaceId == 11630038968) and functions.isAlive(lplr, true) then 
	...
	if mobs then 
		for i, entdata in entities do 
			for i2, v in entdata do 
				if functions.isAlive(lplr, true) and functions.isAlive({Character = v}) then 
...
functions.errorNotification = function(title: string, text: string, delay: number): Frame
	local success, notification = pcall(function()
		local notif = shared.rendervape.CreateNotification(title, text, delay or 6.5, 'assets/WarningNotification.png')
		notif.IconLabel.ImageColor3 = Color3.new(220, 0, 0)
		notif.Frame.Frame.ImageColor3 = Color3.new(220, 0, 0)
		return notif
	end)
	return success and notification
end
